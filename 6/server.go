...
<form action="{{$.DeleteBase}}" method="POST" style="display:inline"><input type="hidden" name="action" value="delete"><input type="hidden" name="id" value="{{.ID}}"><button type="submit" name="delete" value="1">Удалить</button></form></td>
</tr>
{{end}}
</tbody>
</table>
...
<body>
<h1>Редактирование анкеты #{{.App.ID}}</h1>
...
func runCGI() {
	...
	if pathInfo == "" && os.Getenv("REQUEST_URI") != "" {
		...
		pathInfo = strings.TrimPrefix(strings.TrimPrefix(requestURI, base), "/")
...
func serveAdmin(w http.ResponseWriter, r *http.Request) {
	...
	// В CGI форма и редиректы должны вести на скрипт (SCRIPT_NAME), иначе POST при открытии по /6/ уходит не в скрипт
	if os.Getenv("GATEWAY_INTERFACE") != "" {
		if scriptName := os.Getenv("SCRIPT_NAME"); scriptName != "" {
			selfPath = scriptName
		}
	}
	if r.Method == http.MethodPost {
		_ = r.ParseForm()
		act := strings.TrimSpace(r.FormValue("action"))
		if act == "delete" || strings.TrimSpace(r.FormValue("delete")) == "1" {
			handleDelete(w, r, base, selfPath)
			return
		}
		if r.FormValue("id") != "" {
			handleEdit(w, r, base, selfPath)
			return
		}
	}
	if r.Method == http.MethodGet && r.URL.Query().Get("edit") != "" {
		handleEdit(w, r, base, selfPath)
		return
	}
	if r.Method == http.MethodGet {
		handleList(w, r, base, selfPath)
		return
	}
	http.Error(w, "Method Not Allowed", http.StatusMethodNotAllowed)
}

func handleList(w http.ResponseWriter, r *http.Request, base, selfPath string) {
	w.Header().Set("Content-Type", "text/html; charset=utf-8")
	apps, err := listAllApplications()
	if err != nil {
		log.Printf("list applications: %v", err)
		// Понятное сообщение: часто таблица application в другой БД или не создана
		errMsg := template.HTMLEscapeString(err.Error())
		w.WriteHeader(http.StatusInternalServerError)
		w.Write([]byte(`<!DOCTYPE html><html lang="ru"><head><meta charset="UTF-8"><title>Ошибка</title></head><body><h1>Ошибка загрузки</h1><p>Не удалось загрузить список заявок. Убедитесь, что в той же базе данных, к которой подключается панель администратора, созданы таблицы <code>application</code>, <code>application_language</code>, <code>programming_language</code> (как в задании 5).</p><pre>`))
		w.Write([]byte(errMsg))
	...
	case "deleted":
		listMsg = "Заявка удалена."
	case "saved":
		listMsg = "Данные сохранены."
	}
	data := struct {
		Apps       []appRecord
		Stats      []langStat
		...
		EditBase   string
		DeleteBase string
	}{Apps: apps, Stats: stats, Message: listMsg, EditBase: appendQuery(selfPath, "edit", "1"), DeleteBase: selfPath}
	if err := listTpl.Execute(w, data); err != nil {
		log.Printf("list template: %v", err)
	}
}

func handleEdit(w http.ResponseWriter, r *http.Request, base, selfPath string) {
	w.Header().Set("Content-Type", "text/html; charset=utf-8")
	...
	if r.Method == http.MethodPost {
		if err := r.ParseForm(); err != nil {
			renderEditForm(w, id, formData{}, map[string]string{"_": "Ошибка формы."}, selfPath)
			return
		}
		form := formData{
		...
		if len(errors) > 0 {
			app := getAppByID(id)
			if app != nil {
				renderEditFormWithApp(w, id, *app, form, errors, selfPath)
			} else {
				redirectTo(w, r, selfPath)
			}
			return
		...
		if err := updateApplication(id, form); err != nil {
			...
			renderEditForm(w, id, form, map[string]string{"_": "Ошибка сохранения."}, selfPath)
			return
		}
		redirectTo(w, r, appendQuery(selfPath, "msg", "saved"))
		return
	}
	...
	renderEditFormWithApp(w, id, *app, form, nil, selfPath)
}
...
func appendQuery(path, key, value string) string {
	...
	if strings.Contains(path, "?") {
		return path + "&" + key + "=" + url.QueryEscape(value)
	}
	return path + "?" + key + "=" + url.QueryEscape(value)
}
...
func handleDelete(w http.ResponseWriter, r *http.Request, base, selfPath string) {
	if r.Method == http.MethodPost {
		_ = r.ParseForm()
	}
	idStr := r.URL.Query().Get("id")
	...
	redirectTo(w, r, appendQuery(selfPath, "msg", "deleted"))
}
...
func listAllApplications() ([]appRecord, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&a.ID, &login, &a.Fio, &a.Phone, &a.Email, &birthdate, &a.Gender, &bio, &agreed); err != nil {
			return nil, err
		}
		a.Birthdate = birthdate.Format("2006-01-02")
		if login.Valid {
...
func getAppByID(id int64) *appRecord {
	...
	var agreed int
	var birthdate time.Time
	err := db.QueryRow("SELECT id, login, fio, phone, email, birthdate, gender, biography, contract_agreed FROM application WHERE id=?", id).Scan(&a.ID, &login, &a.Fio, &a.Phone, &a.Email, &birthdate, &a.Gender, &bio, &agreed)
	if err != nil {
		return nil
	}
	a.Birthdate = birthdate.Format("2006-01-02")
...
func loadFormByAppID(appID int64) formData {
	...
	var agreed int
	var birthdate time.Time
	err := db.QueryRow("SELECT fio, phone, email, birthdate, gender, biography, contract_agreed FROM application WHERE id=?", appID).Scan(&f.Fio, &f.Phone, &f.Email, &birthdate, &f.Gender, &bio, &agreed)
	if err != nil {
		return formData{}
	}
	f.Birthdate = birthdate.Format("2006-01-02")
	if bio.Valid {
...
func renderEditForm(w http.ResponseWriter, id int64, form formData, errors map[string]string, backURL string) {
	app := getAppByID(id)
	if app == nil {
		return
	}
	renderEditFormWithApp(w, id, *app, form, errors, backURL)
}

func renderEditFormWithApp(w http.ResponseWriter, id int64, app appRecord, form formData, errors map[string]string, backURL string) {
	if errors == nil {
	...
	wrap := editDataWithBack{editData: data, BackURL: backURL}
