...
<form action="{{$.DeleteBase}}" method="POST" style="display:inline" onsubmit="return confirm('Удалить заявку?')"><input type="hidden" name="action" value="delete"><input type="hidden" name="id" value="{{.ID}}"><button type="submit">Удалить</button></form></td>
</tr>
{{end}}
</tbody>
</table>
...
func serveAdmin(w http.ResponseWriter, r *http.Request) {
	...
	_ = user
	base := getBasePath()
	pathOK := r.URL.Path == base || r.URL.Path == base+"/"
	if pathOK {
...
func handleList(w http.ResponseWriter, r *http.Request, base string) {
	...
	data := struct {
		...
		EditBase   string
		DeleteBase string
	}{Apps: apps, Stats: stats, Message: msg, EditBase: base + "/?edit=1", DeleteBase: base + "/"}
