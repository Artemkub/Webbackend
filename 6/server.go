...
func handleList(w http.ResponseWriter, r *http.Request, base, selfPath string) {
	...
	data := struct {
		...
		EditBase   string
		DeleteBase string
	}{Apps: apps, Stats: stats, Message: msg, EditBase: selfPath + "?edit=1", DeleteBase: selfPath}
...
func handleEdit(w http.ResponseWriter, r *http.Request, base, selfPath string) {
	...
	if r.Method == http.MethodPost {
		...
		redirectTo(w, r, selfPath+"?msg=saved")
		return
	}
...
func handleDelete(w http.ResponseWriter, r *http.Request, base, selfPath string) {
	...
	redirectTo(w, r, selfPath+"?msg=deleted")
}
...
func listAllApplications() ([]appRecord, error) {
	...
	for rows.Next() {
		...
		var agreed int
		if err := rows.Scan(&a.ID, &login, &a.Fio, &a.Phone, &a.Email, &a.Birthdate, &a.Gender, &bio, &agreed); err != nil {
			return nil, err
		}
		if login.Valid {
...
func getAppByID(id int64) *appRecord {
	...
	var bio sql.NullString
	var agreed int
	err := db.QueryRow("SELECT id, login, fio, phone, email, birthdate, gender, biography, contract_agreed FROM application WHERE id=?", id).Scan(&a.ID, &login, &a.Fio, &a.Phone, &a.Email, &a.Birthdate, &a.Gender, &bio, &agreed)
	if err != nil {
		return nil
	}
	if login.Valid {
