...
func runCGI() {
	...
	for _, line := range w.header["Set-Cookie"] {
		out.Write([]byte("Set-Cookie: " + line + "\r\n"))
	}
	...
	if vals["from"] == "error" {
		...
		for k, v := range vals {
			if strings.HasPrefix(k, "err_") && v != "" {
				errors[strings.TrimPrefix(k, "err_")] = v
			}
		}
	}
	form.Fio = vals["fio"]
	form.Phone = vals["phone"]
	form.Email = vals["email"]
	form.Birthdate = vals["birthdate"]
	form.Gender = vals["gender"]
	form.Biography = vals["bio"]
	form.Agreement = vals["agreement"] == "on"
	if s := vals["languages"]; s != "" {
...
func setSessionErrorCookies(w http.ResponseWriter, form formData, errors map[string]string) {
	...
	for k, v := range errors {
		...
		http.SetCookie(w, &http.Cookie{Name: cookiePrefix + "err_" + k, Value: v, Path: path})
	}
}
...
func setFormCookies(w http.ResponseWriter, form formData, maxAge int) {
	...
	for _, o := range opts {
		c := &http.Cookie{Name: cookiePrefix + o.name, Value: o.val, Path: path}
		if maxAge > 0 {
			c.MaxAge = maxAge
		}
		http.SetCookie(w, c)
