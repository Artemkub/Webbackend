...
{{end}}
{{if .Auth}}
<p><a href="{{.LoginPath}}">Войти</a> под другой учётной записью | <a href="{{.LogoutPath}}">Выйти</a></p>
{{else}}
<p><a href="{{.LoginPath}}">Войти</a> для редактирования ранее отправленных данных</p>
{{end}}
...
func handleForm(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == http.MethodGet {
		if auth {
			...
			if form != nil {
				renderForm(w, pageData{Form: *form, Auth: true, LoginPath: loginPath, LogoutPath: logoutPath})
				return
			}
		}
		...
		if fromError {
			renderForm(w, pageData{Form: form, Errors: errors, Message: msg, LoginPath: loginPath, LogoutPath: logoutPath})
			deleteErrorCookies(w)
			return
		}
		if msg != "" {
			renderForm(w, pageData{Form: form, Message: msg, LoginPath: loginPath, LogoutPath: logoutPath})
			deleteSuccessMessageCookie(w)
			return
		}
		renderForm(w, pageData{Form: form, LoginPath: loginPath, LogoutPath: logoutPath})
		return
	}
	...
	if err := r.ParseForm(); err != nil {
		renderForm(w, pageData{Form: formData{}, Errors: map[string]string{"_": "Ошибка формы."}, LoginPath: loginPath, LogoutPath: logoutPath})
		return
	}
	form := formData{
	...
	setSuccessMessageCookie(w, "Данные сохранены. Логин: "+login+" Пароль: "+password+" — сохраните их для входа и редактирования.")
	redirectGET(w, r, "")
}
...
func readSuccessMessageCookie(r *http.Request) template.HTML {
	for _, c := range r.Cookies() {
		if c.Name == cookiePrefix+"msg" && c.Value != "" {
			return template.HTML(template.HTMLEscapeString(cookieDecode(c.Value)))
		}
	}
	return ""
}
...
func renderForm(w http.ResponseWriter, data pageData) {
	...
	if data.LoginPath == "" {
		data.LoginPath = getBasePath() + "/login"
	}
	if data.LogoutPath == "" {
		data.LogoutPath = getBasePath() + "/logout"
	}
